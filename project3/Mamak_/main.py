...
def replace(word, letter, index, current_word):  # Méthode qui permet de remplacer un '_' dans un word par la letter à l'index + affiche le résultat
	index_array=[0, 1]
	index_array=index_array.append(index)
	wordarray=list(word)
	current_wordarray=list(current_word)
	replacing=1
	while len(index_array) <= replacing:
    	replace_index = index[replacing]
    	replace_letter = wordarray[replace_index]
    	current_wordarray[replace_index]=replace_letter    
    	current_word=current_word.join(current_wordarray)
